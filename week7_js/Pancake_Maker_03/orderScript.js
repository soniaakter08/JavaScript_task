document.addEventListener("DOMContentLoaded", () => {
  ...
  function createDropDownList(parentElement, order) {
    ...
    optionsOfStatus.forEach((option) => {
      ...
      if (order.status === "Waiting") {
        parentElement.classList.add("status-waiting");
      }
      if (order.status === "Ready") {
        parentElement.classList.add("status-ready");
      }
      if (order.status === "Delivered") {
        parentElement.classList.add("status-delivered");
      }
    });
  ...
  sortOrderButton.addEventListener("click", () => {
    const statusOrder = ["waiting", "ready", "delivered"];

    ordersObject.forEach((order) => {
      order.status = order.status.trim().toLowerCase();
    });
